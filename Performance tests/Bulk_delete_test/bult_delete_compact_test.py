...
4. Wait WAIT_SECONDS (default 300 s / 5 min)
5. Bulk-delete all documents
6. Capture storage stats  →  "AFTER DELETE  (immediate)"
7. (Optional) Run compact()  →  "AFTER COMPACT"
8. Write JSON report
...
DEFAULT_WAIT_SECONDS = 240          # 5 minutes between insert and delete
...
def get_collection_stats(db, coll_name: str) -> Dict[str, Any]:
    """
    Pull storage statistics for the collection.
    Returns a dict with human-readable and raw byte values.
    """
    raw = db.command("collStats", coll_name)
    ...
    stats = {
        ...
        "wt_bytes_available_reuse_human":
            fmt_bytes(bm.get("file bytes available for reuse", 0)),
...
def insert_documents(collection, total: int, batch_size: int) -> float:
    ...
    elapsed = time.perf_counter() - start_time
    print(f"\n\n  ✅  Inserted {inserted:,} docs in {elapsed:.1f}s  "
          f"({inserted/elapsed:,.0f} docs/s)")
    return elapsed
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--total-docs",    default=DEFAULT_TOTAL_DOCS,   type=int,
                   help=f"Total documents to insert (default: {DEFAULT_TOTAL_DOCS:,})")
    p.add_argument("--batch-size",    default=DEFAULT_BATCH_SIZE,   type=int,
                   help=f"insertMany batch size (default: {DEFAULT_BATCH_SIZE:,})")
    p.add_argument("--wait",          default=DEFAULT_WAIT_SECONDS, type=int,
                   help=f"Seconds to wait before delete (default: {DEFAULT_WAIT_SECONDS})")
    p.add_argument("--run-compact",   action="store_true",          default=DEFAULT_RUN_COMPACT,
                   help="Run compact() after delete and capture stats")
    p.add_argument("--auto-compact",  action="store_true",          default=DEFAULT_AUTO_COMPACT,
    ...
    p.add_argument("--drop-first",    action="store_true",
                   help="Drop existing collection before test")
...
def main() -> None:
    ...
    stats_after_insert = get_collection_stats(db, args.collection)
    ...
    # ── Stats: immediately after delete ──────────────────────
    print("\n[7/7] Capturing storage stats AFTER DELETE …")
    stats_after_delete = get_collection_stats(db, args.collection)
    ...
    if args.run_compact:
        ...
        try:
            ...
            stats_after_compact = get_collection_stats(db, args.collection)
    ...
    # ── Tips for initial sync comparison ─────────────────────
    print("""
